...
const replaceElementChild = (element: HTMLElement, childToReplace: HTMLElement, newChild: HTMLElement) => {
  element.removeChild(childToReplace)
  element.appendChild(newChild)
}

const unmountElement = (element: HTMLElement) => element.parentNode?.removeChild(element)
...
export const createSidetab = (formId: string, options: SidetabOptions = defaultOptions): Sidetab => {
  ...
  iframe.onload = () => {
    setTimeout(() => {
      ...
      replaceElementChild(button, spinner, closeIcon)
    }, 250)
  }
  ...
  const open = () => {
    if (!isOpen(wrapper)) {
      ...
      replaceElementChild(button, icon, spinner)
    }
  }
  ...
  const close = () => {
    if (isOpen(wrapper)) {
      setTimeout(() => {
        ...
        replaceElementChild(button, closeIcon, icon)
      }, 250)
    }
  }
